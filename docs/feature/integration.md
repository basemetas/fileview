...
const opts = {
  ...
  fileName: "sample.docx", // 真实文件名，用作文件类型判断，如果文件地址中没有正确必须
  displayName: "网络示例文档", // 用于标题栏等展示的文件名，非必需
};

// 对参数进行base64编码
const base64 = encodeURIComponent(Base64.encode(JSON.stringify(opts)));

// 构造预览地址
const previewUrl = `https://yourPreviewService/preview/view?data=${base64}`;
